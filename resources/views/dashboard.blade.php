...
@section('content')
    <section class="page-intro">
        <div>
            ...
            <p class="page-description">
                Your account summary and recent URL activity. Use the navigation to manage each module on its own page.
            </p>
        </div>

        <time class="date-chip" datetime="{{ now()->toDateString() }}">
            {{ now()->format('l, d M Y') }}
        </time>
    </section>
    ...
    <section class="quick-grid" aria-label="Quick actions">
        <a class="quick-card" href="{{ route('short-urls.index') }}">
            ...
            <div>
                <p class="quick-title">{{ $user->isSuperAdmin() ? 'Browse all URLs' : 'Manage short URLs' }}</p>
                <p class="quick-copy">Open the dedicated URL directory and review click totals.</p>
            </div>
        </a>
        ...
        @if ($user->isSuperAdmin())
            <a class="quick-card" href="{{ route('companies.index') }}">
                <span class="quick-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                        ...
                        <path d="M8 10h2m4 0h2m-8 4h2m4 0h2M2 21h20" />
                    </svg>
                </span>
                <div>
                    <p class="quick-title">Manage companies</p>
                    <p class="quick-copy">Create companies and invite or review their administrators.</p>
                </div>
            </a>
            ...
            <a class="quick-card" href="{{ route('super-admin.analytics') }}">
                ...
                <div>
                    <p class="quick-title">Open analytics</p>
                    <p class="quick-copy">Filter companies, users, URLs and clicks in one report.</p>
                </div>
            </a>
        @elseif ($user->isAdmin())
            <a class="quick-card" href="{{ route('team.index') }}">
                ...
                <div>
                    <p class="quick-title">Manage my team</p>
                    <p class="quick-copy">Create Admin or Member accounts and resend invitations.</p>
                </div>
            </a>
            ...
            <a class="quick-card" href="{{ route('admin.analytics') }}">
                ...
                <div>
                    <p class="quick-title">Open analytics</p>
                    <p class="quick-copy">Compare your URLs with users created by you.</p>
                </div>
            </a>
        @endif
    </section>
    ...
    <div class="overview-grid">
        <section class="panel">
            <header class="panel-header">
                <div>
                    <h2 class="panel-title">Recent short URLs</h2>
                    <p class="panel-subtitle">The latest five links in your current scope.</p>
                </div>
            ...
            <div class="table-wrap">
                <table class="data-table">
                    ...
                    @forelse ($recentShortUrls as $shortUrl)
                        <tr>
                            <td class="url-cell">
                                <a class="short-link" href="{{ $shortUrl->shortUrl() }}" target="_blank" rel="noopener">
                                    {{ $shortUrl->shortUrl() }}
                                </a>
                            </td>
                            <td class="url-cell">
                                <a class="long-link" href="{{ $shortUrl->original_url }}" target="_blank" rel="noopener">
                                    {{ $shortUrl->original_url }}
                                </a>
                            </td>
                            @if ($user->isSuperAdmin())
                                <td>
                                    <p class="person-name">{{ $shortUrl->user->name }}</p>
                                    <p class="person-email">{{ $shortUrl->user->company?->name ?? 'No company' }}</p>
                                </td>
                            @endif
                    ...
                    @empty
                        <tr>
                            <td class="empty-state" colspan="{{ $user->isSuperAdmin() ? 4 : 3 }}">
        ...
        <aside class="panel">
            <header class="panel-header">
                <div>
                    ...
                    <p class="panel-subtitle">Your current workspace identity.</p>
                </div>
            </header>
            ...
            <div class="panel-body">
                <dl class="profile-list">
                    ...
                    <div class="profile-row">
                        ...
                        <dd class="profile-value">{{ $user->email }}</dd>
                    </div>
                    <div class="profile-row">
                        <dt class="profile-label">Role</dt>
                        <dd class="profile-value">{{ ucwords(str_replace('_', ' ', $user->role->value)) }}</dd>
                    </div>
                    @if ($user->company)
                        <div class="profile-row">
                            <dt class="profile-label">Company</dt>
                            <dd class="profile-value">{{ $user->company->name }}</dd>
                        </div>
                    @endif
                </dl>
            </div>
